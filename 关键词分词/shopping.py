...
def select_data_from_mysql():
    ...
    cursor.execute("SELECT app_name,download_time,download_num,good,comment_number,comment_detail from shopping")
...
def write_comment_to_mysql(i):
    ...
    # SQL 插入语句
    sql = 'INSERT INTO shopping(app_id,app_name,download_time,download_num,good,comment_number,comment_detail) VALUES (%s,%s,%s,%s,%s,%s,%s)' % (app_id,app_name,download_num,good,comment_number,comment_detail,download_time)
    #print(sql)
    try:
        # 执行sql语句
        cur.execute(sql)
