...
def main():
   ...
   plt.show()
   plt.savefig(r'C:\Users\Administrator\Desktop\The last trip\折线图表分析\communicate_line.png')
   
   
def select_data_from_mysql():
    conn = pymysql.connect(host='127.0.0.1', port=3306, user='root', passwd='123456',db='final' )
    sql_query = 'SELECT app_name,comment_number from total WHERE tag="通讯社交" '
