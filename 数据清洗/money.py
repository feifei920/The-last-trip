...
def select_data_from_mysql():
    ...
    cursor.execute('SELECT app_name,download_time,download_num,good,comment_number,comment_detail FROM total WHERE tag="金融理财" AND download_time=%s',(localtime)) 
...
def write_to_mysql(i):
    ...
    # SQL 插入语句
    sql = 'INSERT INTO money(app_id,app_name,download_time,download_num,good,comment_number,comment_detail) VALUES (%s, %s, %s, %s, %s, %s, %s)' % (app_id,app_name,download_time,download_num,good, comment_number,comment_detail)
    try:
        # 执行sql语句
        cur.execute(sql)
        # 提交到数据库执行
